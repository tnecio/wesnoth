...
// Headless/WASM build: no-op stubs for all display functions.
...
public:
    ...
    void start(unit_ptr) {}
    void proceed_to(unit_ptr, std::size_t, bool = false, bool = true) {}
    void wait_for_anims() {}
    void finish(unit_ptr, map_location::direction = map_location::direction::indeterminate) {}
};

inline void move_unit(const std::vector<map_location>&, const unit_ptr&,
...
inline void unit_draw_weapon(const map_location&, unit&,
    const const_attack_ptr& = nullptr, const const_attack_ptr& = nullptr,
    const map_location& = map_location::null_location(), const unit_ptr& = unit_ptr()) {}

inline void unit_sheath_weapon(const map_location&, const unit_ptr& = unit_ptr(),
    const const_attack_ptr& = nullptr, const const_attack_ptr& = nullptr,
    const map_location& = map_location::null_location(), const unit_ptr& = unit_ptr()) {}
...
inline void unit_attack(display*, game_board&,
    ...
    const attack_type&, const const_attack_ptr&,
